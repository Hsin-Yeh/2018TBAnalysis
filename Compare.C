...
void Compare(){
  ...
  sprintf(title,"root_plot/MC_20GeV_ntuple_sim_pdgID11_beamMomentum20.0_listFTFP_BERT_EMM_CrossTalk_0.00173Threshold_SameTotalE_0.05Xfactor_result.root");
  TFile f_MC(title);
  sprintf(title,"root_plot/MC_20GeV_ntuple_sim_pdgID11_beamMomentum20.0_listFTFP_BERT_EMM_CrossTalk_0.00173Threshold_SameTotalE_0.01Xfactor_result.root");
  TFile f_MC_CrossTalk(title);
  sprintf(title,"root_plot/MC_20GeV_ntuple_sim_pdgID11_beamMomentum20.0_listFTFP_BERT_EMM_CrossTalk_0.00173Threshold_SameTotalE_0.005Xfactor_result.root");
  ...
  legend->AddEntry(h_totalCEE[1],"XTalk","L");
  legend->AddEntry(h_totalCEE[2],"XTalk,Th=1.73e-3","L");
  legend->AddEntry(h_totalCEE[3],"Data","LP");
  legend->SetTextSize(0.035);
  legend->Draw();
  c1->Update();
  gPad->WaitPrimitive();
  sprintf(title,"Total Energy CEE.pdf");
  c1->SaveAs(title);
  delete legend;
  ...
  for(int iL = 0; iL < NLAYER ; ++iL){
    ...
    legend->AddEntry(h_E1devE7[0][iL],"No XTalk","L");
    legend->AddEntry(h_E1devE7[1][iL],"XTalk","L");
    legend->AddEntry(h_E1devE7[2][iL],"XTalk,Th=1.73e-3","L");
    legend->AddEntry(h_E1devE7[3][iL],"Data","LP");
    legend->SetTextSize(0.035);
    legend->Draw();
    c1->Update();
    gPad->WaitPrimitive();
    sprintf(title,"layer%d_E1devE7.pdf",iL);
    c1->SaveAs(title);
    ...
    c1->Update();
    gPad->WaitPrimitive();
    sprintf(title,"layer%d_E7devE19.pdf",iL);
    c1->SaveAs(title);
